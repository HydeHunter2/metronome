...
class MainTabBarPresenter: MainTabBarPresenterProtocol, ParentOfEditableTablePresenterProtocol, ParentOfCollectionPresenterProtocol, ParentOfMetronomePresenterProtocol, ParentOfSettingsPresenterProtocol {
    ...
    var unwindFunctionFromSettings: ((_ data: Preset) -> ())?
    func unwindFromSettings(withData data: Preset) {
        guard let unwind = unwindFunctionFromSettings else { return }
        unwind(data)
    }
