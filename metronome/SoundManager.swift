...
class SoundManager: SoundManagerProtocol {
    ...
    private func syncPlaySound(_ sound: Sound) {
        ...
        guard let player = AVAudioPlayer.createPlayerFromFile(withName: sound.name) else { return }
    ...
    private func syncPlayIntro(withTickDuration tickDuration: Double) {
        for _ in 0...3 {
            guard let player = AVAudioPlayer.createPlayerFromFile(withName: Instrument.hi_hat.name()) else { continue }
            players.append(player)
        }
...
extension AVAudioPlayer {
    static func createPlayerFromFile(withName name: String) -> AVAudioPlayer? {
        if name == GlobalSettings.STRING_OF_NILDATA { return nil }
