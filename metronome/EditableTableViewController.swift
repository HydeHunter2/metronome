...
class EditableTable: UIViewController {
  ...
  @objc private func openCollection(_ sender: UIButton) {
    ...
    presenter?.openCollection(forEditingCellWithPath: tableView.indexPath(for: cell)!)
  }

  @objc func playSound(_ sender: UIButton) {
    let cell = sender.superview?.superview?.superview as! UITableViewCell
    guard let soundName = presenter?.getNameForCell(withPath: tableView.indexPath(for: cell)!) else {
      return
    }
