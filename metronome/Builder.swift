...
class Builder {
  static func createMainTabBarModule() -> UITabBarController {
    ...
    let presenter = MainTabBarPresenter(controller: controller,
                                        metronome: metronomeModule.presenter,
                                        settings: settingsModule.presenter,
                                        instrument: instrumentModule.presenter,
                                        note: noteModule.presenter,
                                        collection: collectionModule.presenter,
  ...
  static func createMetronomeModule() -> (presenter: MetronomePresenter, view: MetronomeViewController) {
    ...
    for tick in 0..<GlobalSettings.STANDART_NUMBER_OF_TICKS {
      model.beats += [(instruments: GlobalSettings.STANDART_INSTRUMENTS[tick],
                       notes: GlobalSettings.STANDART_NOTES[tick])]
    }
