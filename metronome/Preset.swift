...
class Preset: NSObject, NSCoding, StorageObjectProtocol {
    ...
    static let empty = Preset(title: GlobalSettings.NAME_OF_UNTITLED_PRESET, beats: [], BPM: GlobalSettings.MIN_BPM)
    ...
    required convenience init?(coder: NSCoder) {
        ...
        for tick in beatsString.split(separator: "|") {
            ...
            let instruments = tick.split(separator: ":")[0]
                                  .split(separator: ",")
                                  .map{ name in Instrument.getData(withName: String(name)) as! Instrument? }
            
            let notes = tick.split(separator: ":")[1]
                            .split(separator: ",")
                            .map{ name in Note.getData(withName: String(name)) as! Note? }
